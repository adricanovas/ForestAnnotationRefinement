...
class InsSegTester(TesterBase):
    ...
    def test(self):
        assert self.test_loader.batch_size == 1
        logger = get_root_logger()
        # Change log message to reflect that we are doing inference and saving .pth files, not evaluating metrics
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            # We added a check for number points to avoid errors
            if data_dict['coord'].shape[0] < 4000:
                logger.info(
                    "Skipping {} [{}/{}] due to insufficient points ({} points)".format(
                        data_name, idx + 1, len(self.test_loader), data_dict['coord'].shape[0]
                    )
                )
                continue
            # -------------------------------------------------------------
            ...
            with torch.no_grad():
                ...
                torch.save(output_dict, os.path.join(save_path, f"{data_name}_pred.pth"))
        ...
        logger.info("<<<<<<<<<<<<<<<<< Inference Completed (Saved .pth) <<<<<<<<<<<<<<<<<")
