...
class PointGroup(nn.Module):
    ...
    def forward(self, data_dict, return_point=False):
        ...
        if not self.training:
            ...
            # CHANGED: We create an empty proposals_pred tensor at the beginning to avoid errors when there are no proposals detected.
            proposals_pred = torch.zeros(
                (proposals_offset.shape[0] - 1, center_pred.shape[0]), dtype=torch.int
            )

            # Changed: We check if proposals_idx has valid dimensions and is not empty before trying to index into proposals_pred and segment_pred. 
            # If there are no proposals, we skip this block and keep pred_scores, pred_classes, and pred_masks as empty tensors.
            if proposals_idx.ndim > 1 and proposals_idx.shape[0] > 0:
                ...
                # If there are no valid proposals, we set pred_scores, pred_classes, and pred_masks to empty tensors to avoid errors in the rest of the code.
                pred_scores = torch.tensor([])
                pred_classes = torch.tensor([])
                pred_masks = torch.zeros((0, center_pred.shape[0]), dtype=torch.int)

            # Change: We save the results and the original coordinates in the return_dict, which will be saved as .pth files for later evaluation.
            ...
            if "origin_coord" in data_dict.keys():
                return_dict["coord"] = data_dict["origin_coord"].cpu()
                return_dict["inverse"] = data_dict["inverse"].cpu()
        return return_dict
